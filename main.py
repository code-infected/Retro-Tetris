...
class Tetris:
    """Class to represent the Tetris game."""
    ...
    def new_piece(self) -> List[List[int]]:
        """Returns a new random Tetris piece."""
    ...
    def valid_move(self, piece: List[List[int]], offset: Tuple[int, int]) -> bool:
        """Checks if moving the piece to the offset is valid."""
        off_x, off_y = offset
...
class GameState:
    ...
    def __init__(self):
        ...
        self.speed = 500  # Lower value means faster game
    ...
    def quit_confirmation(self):
        """Handles the quit confirmation state."""
        while self.state == "quit_confirmation":
            screen.fill(BLACK)
    ...
    def game(self):
        ...
        while self.state == "game":
            ...
            for y, row in enumerate(self.tetris.board):
                for x, cell in enumerate(row):
                    if cell:
                ...
                for x, cell in enumerate(row):
                    if cell:
                        pygame.draw.rect(screen, WHITE, (
                            (self.tetris.piece_x + x) * BLOCK_SIZE, (self.tetris.piece_y + y) * BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE))

            pygame.display.update()
            self.clock.tick(30)

    def run(self):
        """Runs the game state manager."""
        while True:
            if self.state == "main_menu":
                self.main_menu()
            elif self.state == "settings":
                self.settings()
            elif self.state == "quit_confirmation":
                self.quit_confirmation()
            elif self.state == "game":
                self.game()


def draw_text(text: str, font: pygame.font.Font, color: Tuple[int, int, int], surface: pygame.Surface, x: int, y: int):
